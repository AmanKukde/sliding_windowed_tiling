...
# %%
from careamics.lvae_training.eval_utils import stitch_and_crop_predictions_inner_tile
device = get_device()
...
with torch.no_grad():
    for batch in tqdm(dloader, desc="Predicting tiles"):
        ...
        tile_mmse.append(rec.cpu().numpy())
tiles_arr = np.concatenate(tile_mmse, axis=0)
print(len(tiles_arr))
# import dill
# from usplit.core.tiff_reader import save_tiff
# save_tiff("/home/aman.kukde/Projects_2/Results/Results_21Jul2025/tiles_arr_test_dset_pavia_usplit_windowed.tiff",tiles_arr)
# with open('/group/jug/aman/results/tiles_arr_test_dset_pavia_usplit_windowed.pkl',"wb") as f:
#     dill.dump(tiles_arr,f)

stitch_func = stitch_and_crop_predictions_inner_tile
print(f"\n\nUsing {stitch_func}\n\n")

stitched_predictions, counts_matrix_for_stitched_predictions = stitch_func(tiles_arr, test_dset)
...
save_tiff(f'/group/jug/aman/results/Results_usplit_64/{dataset}/pred_test_dset_usplit_windowed.tiff',stitched_predictions.transpose(0,3,1,2))
with open(f'/group/jug/aman/results/Results_usplit_64/{dataset}/pred_test_dset_usplit_windowed.pkl',"wb") as f:
